...
#'@return
#'
mgplot <- function(srlist, chr, add2existingplot=F,
  ...
  ### read data
  sr <- read.delim(srlist)
  ncomparisons <- nrow(sr) # number of syri comparisons
  
  ### chromosome lengths and the maxial lengths from all 
  current_chrlens <- NULL
  for (i in 1:ncomparisons) {
    genome_name_ref <- sr[i, 2]
    genome_name_qry <- sr[i, 5]
    
    chrlengths_ref <- read.delim(sr[i, 3], stringsAsFactors=F, header=F)
    chrlen_ref <- chrlengths_ref[chrlengths_ref[,1]==chr, 2]
    
    chrlengths_qry <- read.delim(sr[i, 6], stringsAsFactors=F, header=F)
    chrlen_qry <- chrlengths_qry[chrlengths_qry[,1]==chr, 2]
    
    if (is.null(current_chrlens) | sum(genome_name_ref %in% current_chrlens[,1])==0) {
      current_chrlens <- rbind(current_chrlens, c(genome_name_ref, chrlen_ref))
    }
    
    if (is.null(current_chrlens) | sum(genome_name_qry %in% current_chrlens[,1])==0) {
      current_chrlens <- rbind(current_chrlens, c(genome_name_qry, chrlen_qry))
    }
  }
  current_chrlens <- data.frame(current_chrlens)
  rownames(current_chrlens) <- 1:nrow(current_chrlens)
  current_chrlens[, 2] <- as.numeric(as.character(current_chrlens[, 2]))
  
  # canvasas (0, 1)
  par(mar=c(2, 0, 0, 0))
  
  # plot xrange and unit
  max.chrlen <- max(current_chrlens[, 2])
  ...
  if (!add2existingplot) {
    plot(NULL, NULL, xlim=c(genome.name.space.len, xright), ylim=c(0, ymax), axes=F, xlab="", ylab="", main="")
  }
  ...
  if (is.null(main.pos)) {
    text(xleft, ymax * 0.96, pos=4, labels=main, cex=1.2, col=main.label.col, xpd=T)
  } else {
  ...
  for (i in 1:ncomparisons) {
    ...
    if (is.null(previous.genome)) {
      ref.chrlen <- current_chrlens[current_chrlens[,1]==ref.name, 2]
      ...
      if (!is.na(ref.bedfile)) {
        chr.add.highlight(bed=ref.bedfile, chr=chr, ypos=ref.ypos, chr.height=chr.height)
      }
    } else if (ref.name != previous.genome) {
      stop("two neighboring SYRI results did not share a common genome\n")
    }
    
    ### qry chromosome
    qry.chrlen <- current_chrlens[current_chrlens[,1]==qry.name, 2]
    ...
    if (!is.na(qry.bedfile)) {
      chr.add.highlight(bed=qry.bedfile, chr=chr, ypos=qry.ypos, chr.height=chr.height)
    }
  ...
  ### legends
  if (band.legend.add) {
    legend(band.legend.xpos, ymax*1.04, legend=band.types, col=band.cols,
           lty=1, bty="n", lwd=3, ncol=2, xpd=T, cex=0.8)
  }
