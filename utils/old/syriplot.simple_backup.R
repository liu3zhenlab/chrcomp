...
plotchr <- args[3] # chromosome to plot
qryname <- args[4] # qry label
refname <- args[5] # ref label
outfile <- args[6] # PDF output file
minsyn <- args[7] # minimal syntenic length
minsyn <- as.numeric(minsyn)
minhdr <- args[8] # minimal HDR length
minhdr <- as.numeric(minhdr)
minothers <- args[9] # minimal length for other types of SV
minothers <- as.numeric(minothers)
maxinvsegratio <- args[10] # maximal length ratio of two inversion sgements
...
#'@return
#'
syriplot <- function(syriout, ref.chrlen, qry.chrlen, chr, ref.name, qry.name, outfile=NULL,
                     ...
                     outpdf=T, pdfwidth=5, pdfheight=3) {
  ...
  if (legend.add) {
    legend(0.45, 1.02, legend=band.types, col=band.cols, lty=1, bty="n", lwd=3, ncol=2, xpd=T, cex=0.8)  
  }
...
ref.label <- paste0(refname, ":", plotchr)
qry.label <- paste0(qryname, ":", plotchr)
syriplot(syriout=syriout, ref.chrlen=reflen, qry.chrlen=qrylen,
         chr=plotchr, ref.name=ref.label, qry.name=qry.label,
