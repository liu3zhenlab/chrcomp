...
syriout <- args[1] # syri output
nucmerout <- args[2] # NUCmer table output that contains reflen and qrylen
qchr <- args[3] # query chromosome to plot
rchr <- args[4] # ref chromosome to plot
qryname <- args[5] # qry label
refname <- args[6] # ref label
outfile <- args[7] # PDF output file
minsyn <- args[8] # minimal syntenic length
minsyn <- as.numeric(minsyn)
minhdr <- args[9] # minimal HDR length
minhdr <- as.numeric(minhdr)
minothers <- args[10] # minimal length for other types of SV
minothers <- as.numeric(minothers)
maxinvsegratio <- args[11] # maximal length ratio of two inversion sgements
maxinvsegratio <- as.numeric(maxinvsegratio)
minINS <- args[12] # minimal INS size for labeling 
minINS <- as.numeric(minINS)
...
#'@param min.notal.hdr.size minimum length of unaligned (highly divergent regions) to be plotted (10000) 
...
#'@return
#'
syriplot <- function(syriout, ref.chrlen, qry.chrlen, qchr, rchr, ref.name, qry.name, outfile=NULL,
                     min.syn.size=10000, min.notal.hdr.size=10000,
					 min.others.size=50000, max.invseg.ratio=3,
                     ...
                     outpdf=T, pdfwidth=5, pdfheight=2.6) {
  ...
  #band.cols <- c("ivory2", "orchid4", "goldenrod3", "darkolivegreen4")
  ...
  abline(v=xaxs.lines.at, col="lightblue2", lwd=1)
  abline(v=xaxs.lines.sub, col="lightblue1", lwd=0.4)
  
  ### qry chromosome
  chrdraw(genome.name=qry.name, chr=qchr, chrlen=qry.chrlen, ypos=qry.ypos, chr.col=chr.col,
          height=chr.height, text.add=text.add, text.yadj.value=text.yadj.value)
  
  ### ref chromosome
  chrdraw(genome.name=ref.name, chr=rchr, chrlen=ref.chrlen, ypos=ref.ypos, chr.col=chr.col,
          height=chr.height, text.add=text.add, text.yadj.value=(-1*text.yadj.value))
  
  ### extract chr syri data
  syrichr <- syri[syri[, 1]==rchr & syri[, 6]==qchr, c(2,3,7,8,9,10,11,12)]
  ...
  ### NOTAL
  seghighlight(syridata=syri, chr=rchr, svtype="NOTAL", genome="ref", min.len=min.notal.hdr.size,
               color=chr.highlight.cols["NOTAL"],ypos=ref.ypos, height=chr.height)
  seghighlight(syridata=syri, chr=qchr, svtype="NOTAL", genome="qry", min.len=min.notal.hdr.size,
               color=chr.highlight.cols["NOTAL"],ypos=qry.ypos, height=chr.height) 
  ### HDR
  seghighlight(syridata=syri, chr=rchr, svtype="HDR", genome="ref", min.len=min.notal.hdr.size,
               color=chr.highlight.cols["HDR"],ypos=ref.ypos, height=chr.height)
  seghighlight(syridata=syri, chr=qchr, svtype="HDR", genome="qry", min.len=min.notal.hdr.size,
               color=chr.highlight.cols["HDR"],ypos=qry.ypos, height=chr.height) 
  
  ### legends
  # link bands (band.cols)
  if (legend.add) {
    legend(0.45, 1.04, legend=band.types, col=band.cols, lty=1, bty="n", lwd=3, ncol=2, xpd=T, cex=0.8)  
  }
...
# run
aln <- read.delim(nucmerout, stringsAsFactor=F)
stopifnot(nrow(aln)>=1)
reflen <- unique(as.numeric(as.character(aln$rlen)))
qrylen <- unique(as.numeric(as.character(aln$qlen)))
ref.label <- paste0(refname, ":", rchr)
qry.label <- paste0(qryname, ":", qchr)
syriplot(syriout=syriout, ref.chrlen=reflen, qry.chrlen=qrylen,
         qchr=qchr, rchr=rchr, ref.name=ref.label, qry.name=qry.label,
		 outfile=outfile, min.syn.size=minsyn, max.invseg.ratio=maxinvsegratio,
		 min.notal.hdr.size=minhdr, min.others.size=minothers)
