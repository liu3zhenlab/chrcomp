...
mchr_nucmerplot <- function(complist, 
  ...
  chrdraw <- function(genome.name, chr, chrlen, ypos, height, chr.col,
                      xaxt=NULL, xaxt.pos=2,
					  text.add=T, text.yadj.value=0, text.col="palevioletred4") {
    # xaxt.pos must be 0, 1, or 2
	# 0: bottom; 1: up; 2: both
	x_start <- xpos.conversion(1)
	x_end <- xpos.conversion(chrlen)
	rect(x_start, ypos-height/2, x_end, ypos+height/2, col=chr.col, border=NA)
    chr.label <- paste(genome.name, chr)
    if (text.add) {
      text(0, ypos+text.yadj.value, labels=chr.label, pos=2, col=text.col, cex=0.9)
    }
    
	# x-axis data: a list with four elements - main, labels, unit, sub
	if (!is.null(xaxt)) {
      main.ticks <- xpos.conversion(xaxt[[1]]) # main
      sub.ticks <- xpos.conversion(xaxt[[4]]) # sub
	  # main_ticks
	  for (emain.tick in main.ticks) {
	    if (emain.tick >= x_start & emain.tick <= x_end) {
	      if (xaxt.pos==0 | xaxt.pos==2) {
		    lines(c(emain.tick, emain.tick), c(ypos-height, ypos-height/2),
		          lend=1, col="lightsteelblue3", lwd=2, xpd=T)
		  }
		  if (xaxt.pos==1 | xaxt.pos==2) {
		    lines(c(emain.tick, emain.tick), c(ypos+height/2, ypos+height),
		          lend=1, col="lightsteelblue3", lwd=2, xpd=T)
	      }
		}
	  }
	  # sub_ticks
	  for (esub.tick in sub.ticks) {
	  	if (esub.tick >= x_start & esub.tick <= x_end) {
		  if (xaxt.pos==0 | xaxt.pos==2) {
		    lines(c(esub.tick, esub.tick), c(ypos-height, ypos-height/2),
		          lend=1, col="lightsteelblue3", lwd=0.8, xpd=T)
		  }

		  if (xaxt.pos==1 | xaxt.pos==2) {
		    lines(c(esub.tick, esub.tick), c(ypos+height/2, ypos+height),
		          lend=1, col="lightsteelblue3", lwd=0.8, xpd=T)
	      }
		}
	  }
  ...
  for (i in 1:ncomparisons) {
	...
	ypos.nudge <- chr.height * 0.4 # text ypos adjusted
    
	### ref chromosome
	chrlengths.ref <- read.delim(comp[i, 3], stringsAsFactors=F, header=F)
	chrlen.ref <- chrlengths.ref[chrlengths.ref[,1]==refchr, 2]
	chrdraw(genome.name=ref.name, chr=refchr, chrlen=chrlen.ref,
	        ypos=ref.ypos, text.yadj.value=ypos.nudge,
			chr.col=chr.col, xaxt=xaxtdata, xaxt.pos=0,
	...
	chrlengths.qry <- read.delim(comp[i, 6], stringsAsFactors=F, header=F)
	chrlen.qry <- chrlengths.qry[chrlengths.qry[,1]==qrychr, 2]
	chrdraw(genome.name=qry.name, chr=qrychr, chrlen=chrlen.qry,
	        ypos=qry.ypos, text.yadj.value= -1 * ypos.nudge,
			chr.col=chr.col, xaxt=xaxtdata, xaxt.pos=1,
            height=chr.height, text.add=T, text.col=genome.label.col)
